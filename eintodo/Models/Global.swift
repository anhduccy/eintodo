...
//Global User Model for navigation
...
extension Date{
    ...
    func createDeadlineTime(inputDate: Date)->Date{
...
class ToDoFilter{
    ...
    func withSelectedListAll(l: ToDoList)->NSPredicate{
        let predicate = NSPredicate(format: "_id == %@", l._id)
        return predicate
    }
    //Just show the to-dos which is not completed yet
    func showNotCompleted()->NSPredicate{
        let predicate = NSPredicate(format: "completed == false")
        return predicate
    }
    //Show all to-dos depending on a given date
    func withSelectedDateAll(d: Date)->NSPredicate{
        let cal = Calendar.current
    ...
    func withSelectedDate(d: Date)->NSPredicate{
        let cal = Calendar.current
        let dateFrom = cal.startOfDay(for: d)
        let dateTo = cal.date(byAdding: .second, value: 60*60*24, to: dateFrom)
